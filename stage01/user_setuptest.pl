...
for ($i=0; $i<$numkeys; $i++) {
    ...
    system("date");
$cmd = "runat $ec2timeout euca-add-keypair mykey$i | grep -v KEYPAIR > mykey$i.priv";
    $rc = system($cmd);
    if ($rc) {
...
for ($i=0; $i<$numgroups; $i++) {
    print "\tadding group...\n";
    system("date");
$cmd = "runat $ec2timeout euca-add-group -d \"group$i\" group$i";
    $rc = system($cmd);
    if ($rc) {
...
for ($i=0; $i<$numgroups; $i++) {
    ...
    system("date");
$cmd = "runat $ec2timeout euca-authorize group$i -P icmp -s 0.0.0.0/0 -t -1:-1";
    $rc = system($cmd);
    if ($rc) {
    ...
    system("date");
$cmd = "runat $ec2timeout euca-authorize group$i -P tcp -p 22 -s 0.0.0.0/0";
    $rc = system($cmd);
    if ($rc) {
	print "ERROR: failed to authorize '$cmd'\n";
	exit(1);
